...
[MCP](https://modelcontextprotocol.io) server exposing the CrossrefRest SDK as
two agent tools — `crossref-rest_list` and `crossref-rest_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/crossref-rest-mcp)
make build

# 2. Provide credentials via the environment
export CROSSREF_REST_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user crossref-rest \
  -- /absolute/path/to/crossref-rest-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./crossref-rest-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// crossref-rest_list: first page of records
{ "entity": "funder" }
{ "entity": "funder", "query": { } }

// crossref-rest_load: one record by id
{ "entity": "funder", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/crossref-rest-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user crossref-rest \
     -- "$PWD"/dist/*/crossref-rest-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `crossref-rest_list` and `crossref-rest_load` tools now appear
   in new sessions. Ask the agent to *"list funder using crossref-rest"*
   and it calls `crossref-rest_list` with `{"entity":"funder"}`.
...
```sh
export CROSSREF_REST_APIKEY=sk_live_xxx            # API key
export CROSSREF_REST_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `crossref-rest_list` tool
...
### Call the `crossref-rest_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `crossref-rest_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `crossref-rest_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CROSSREF_REST_APIKEY` | API key sent with every request. |
| `CROSSREF_REST_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

funder | journal | member | type | work

### Smoke test via HTTP (raw JSON-RPC)

```sh
./crossref-rest-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"crossref-rest_load","arguments":{"entity":"funder","query":{"id":1}}}}'
```
