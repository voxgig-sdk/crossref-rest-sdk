...
type Args struct {
	Entity string         `json:"entity" jsonschema:"funder | journal | member | type | work"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.CrossrefRestSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "crossref-rest_list",
		Description: "List records from CrossrefRest. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "crossref-rest_load",
		Description: "Load a single record from CrossrefRest. " +
...
func runOp(client *sdk.CrossrefRestSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.CrossrefRestSDK, name string) (sdk.CrossrefRestEntity, error) {
	switch strings.ToLower(name) {
	case "funder":
		return client.Funder(nil), nil
	case "journal":
		return client.Journal(nil), nil
	case "member":
