...
local FunderEntity = {}
FunderEntity.__index = FunderEntity


function FunderEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FunderEntity)
  self._name = "funder"
  self._client = client
...
function FunderEntity:get_name()
  return self._name
end


function FunderEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FunderEntity.new(self._client, opts)
end
...
function FunderEntity:mark_deleted()
  self._deleted = true
end


function FunderEntity:deleted()
  return true == self._deleted
end


function FunderEntity:data_set(args)
  if args ~= nil then
...
function FunderEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FunderEntity:match_set(args)
  if args ~= nil then
...
function FunderEntity:match_get()
...
function FunderEntity:stream(action, args, callopts)
...
function FunderEntity:_run_op(ctx, post_done)
  local utility = self._utility
