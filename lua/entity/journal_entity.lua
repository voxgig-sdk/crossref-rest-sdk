...
local JournalEntity = {}
JournalEntity.__index = JournalEntity


function JournalEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, JournalEntity)
  self._name = "journal"
  self._client = client
...
function JournalEntity:get_name()
  return self._name
end


function JournalEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return JournalEntity.new(self._client, opts)
end
...
function JournalEntity:mark_deleted()
  self._deleted = true
end


function JournalEntity:deleted()
  return true == self._deleted
end


function JournalEntity:data_set(args)
  if args ~= nil then
...
function JournalEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function JournalEntity:match_set(args)
  if args ~= nil then
...
function JournalEntity:match_get()
...
function JournalEntity:stream(action, args, callopts)
...
function JournalEntity:_run_op(ctx, post_done)
  local utility = self._utility
