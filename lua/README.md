...
The Lua SDK for the CrossrefRest API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Funder(nil):load({ id = "example_id" }, nil)
...
local result, err = client:CrossrefRest(nil):load(
  { id = "test01" }, nil
)
