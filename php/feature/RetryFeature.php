...
// CrossrefRest SDK retry feature
...
class CrossrefRestRetryFeature extends CrossrefRestBaseFeature
{
    ...
    public function init(CrossrefRestContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CrossrefRestContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CrossrefRestContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CrossrefRestContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
