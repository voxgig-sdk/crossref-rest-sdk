...
class WorkEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CROSSREF_REST_TEST_WORK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function work_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("CROSSREF_REST_TEST_WORK_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["CROSSREF_REST_TEST_WORK_ENTID"]);
