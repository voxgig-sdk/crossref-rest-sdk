...
// CrossrefRest SDK pipeline test
//
...
{
    private static function utility(): CrossrefRestUtility
    {
        return new CrossrefRestUtility();
    }
    ...
    private static function ctx(array $over = []): CrossrefRestContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CrossrefRestContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CrossrefRestOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CrossrefRestError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CrossrefRestOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CrossrefRestMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CrossrefRestOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CrossrefRestMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CrossrefRestOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CrossrefRestMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CrossrefRestSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CrossrefRestMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new CrossrefRestResponse([]),
            'result' => new CrossrefRestResult([]),
        ]);
        [, $err] = CrossrefRestMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new CrossrefRestSpec([]),
            'response' => null,
            'result' => new CrossrefRestResult([]),
        ]);
        [, $err] = CrossrefRestMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CrossrefRestSpec(['step' => 's']),
            'response' => new CrossrefRestResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CrossrefRestResult([]),
        ]);
        [, $err] = CrossrefRestMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CrossrefRestSpec(['step' => 's']),
            'response' => new CrossrefRestResponse(self::resp(200, ['v' => 1])),
            'result' => new CrossrefRestResult([]),
        ]);
        [, $err] = CrossrefRestMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CrossrefRestSpec(['step' => 's']),
            'response' => new CrossrefRestResponse(self::resp(200, ['v' => 2])),
            'result' => new CrossrefRestResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CrossrefRestMakeResponse::call($ctx);
    ...
    {
        $preset = new CrossrefRestResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CrossrefRestSpec([]),
            'response' => new CrossrefRestResponse([]),
            'result' => new CrossrefRestResult([]),
        ...
        [$got, $err] = CrossrefRestMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CrossrefRestResult([])]);
        [, $err] = CrossrefRestMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CrossrefRestSpec([]), 'result' => null]);
        [, $err] = CrossrefRestMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CrossrefRestSpec(['step' => 's']),
            'result' => new CrossrefRestResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CrossrefRestOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CrossrefRestMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CrossrefRestSpec(['step' => 's']),
            'result' => new CrossrefRestResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CrossrefRestOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CrossrefRestMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CrossrefRestResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CrossrefRestSpec([]),
            'result' => new CrossrefRestResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CrossrefRestMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CrossrefRestMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new CrossrefRestError('boom', 'boom');
        $utility->fetcher = function (CrossrefRestContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CrossrefRestSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CrossrefRestMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CrossrefRestContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CrossrefRestMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CrossrefRestResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        CrossrefRestMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CrossrefRestContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CrossrefRestSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CrossrefRestMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CrossrefRestResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CrossrefRestSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CrossrefRestMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CrossrefRestMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CrossrefRestMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CrossrefRestResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CrossrefRestDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CrossrefRestResult(['ok' => false])]);
        $this->expectException(CrossrefRestError::class);
        CrossrefRestDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CrossrefRestResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CrossrefRestMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        CrossrefRestMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CrossrefRestBaseFeature();
        $b = new CrossrefRestBaseFeature();
        CrossrefRestFeatureAdd::call($ctx, $a);
        CrossrefRestFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CrossrefRestBaseFeature
    {
        $f = new CrossrefRestBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        CrossrefRestFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): CrossrefRestContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CrossrefRestSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CrossrefRestPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        CrossrefRestPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new CrossrefRestResponse(['headers' => null]),
            'result' => new CrossrefRestResult([]),
        ]);
        CrossrefRestResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        CrossrefRestResultBody::call($ctx);
