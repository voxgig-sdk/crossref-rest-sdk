...
class JournalEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        ...
        $ent = $testsdk->Journal(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = journal_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "journal." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CROSSREF_REST_TEST_JOURNAL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $journal_ref01_data = null;
        if (count($journal_ref01_data_raw) > 0) {
...
function journal_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/journal/JournalTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["journal01", "journal02", "journal03"] as $k) {
    ...
    $entid_env_raw = getenv("CROSSREF_REST_TEST_JOURNAL_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["CROSSREF_REST_TEST_JOURNAL_ENTID"]);
