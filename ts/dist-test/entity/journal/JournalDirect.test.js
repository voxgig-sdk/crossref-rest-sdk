...
(0, node_test_1.describe)('JournalDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-journal', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-journal', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['CROSSREF_REST_TEST_JOURNAL_ENTID'];
