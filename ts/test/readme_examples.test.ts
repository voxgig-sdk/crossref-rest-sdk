...
//     (\`new CrossrefRestSDK(...)\` / \`CrossrefRestSDK.test(...)\` become
//     \`CrossrefRestSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'CrossrefRestSDK'
...
const TEST_SEED = {"entity":{"funder":{"test01":{"id":"test01"}},"journal":{"test01":{"id":"test01"}},"member":{"test01":{"id":"test01"}},"type":{"test01":{"id":"test01"}},"work":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CrossrefRestSDK(...)\` and any
// \`CrossrefRestSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CrossrefRestSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CrossrefRestSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
