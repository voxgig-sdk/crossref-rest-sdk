# CrossrefRest TypeScript SDK Reference

Complete API reference for the CrossrefRest TypeScript SDK.
...
```ts
new CrossrefRestSDK(options?: object)
```
...
#### `CrossrefRestSDK.test(testopts?, sdkopts?)`
...
**Returns:** `CrossrefRestSDK` instance in test mode.
...
Create a new `Funder` entity instance.
...
Alias for `CrossrefRestSDK.test()`.
...
Return the parent `CrossrefRestSDK` instance.
