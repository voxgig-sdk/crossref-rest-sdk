...
class JournalDirectTest < Minitest::Test
  def test_direct_load_journal
    setup = journal_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-journal", setup[:live] ? "live" : "unit")
    if _should_skip
...
def journal_direct_setup(mockres)
