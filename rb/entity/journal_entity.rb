...
class JournalEntity
  ...
  def make
    opts = @_entopts.dup
    JournalEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CrossrefRestHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Journal, Hash] the current Journal data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CrossrefRestHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Journal fields)
  def match_get
  ...
  #
  # @param reqmatch [JournalLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Journal.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Journal, Hash] the loaded Journal; raises CrossrefRestError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = CrossrefRestHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
