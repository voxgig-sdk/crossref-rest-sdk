...
import (
	...
	omni "github.com/voxgig-sdk/crossref-rest-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CrossrefRestError); is {
				out := map[string]any{
					"name":    "CrossrefRestError",
...
func omniCtx(arg any, client *sdk.CrossrefRestSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CrossrefRestSDK); is {
			client = live
