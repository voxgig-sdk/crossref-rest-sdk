...
func TestFunderEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		funderRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.funder")))
		...
		funderRef01MatchDt0 := map[string]any{
			"id": funderRef01Data["id"],
		}
		funderRef01DataDt0Loaded, err := funderRef01Ent.Load(funderRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		funderRef01DataDt0LoadResult := core.ToMapAny(entityData(funderRef01DataDt0Loaded))
		if funderRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if funderRef01DataDt0LoadResult["id"] != funderRef01Data["id"] {
...
func funderBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CROSSREF_REST_TEST_FUNDER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CROSSREF_REST_TEST_FUNDER_ENTID"])
