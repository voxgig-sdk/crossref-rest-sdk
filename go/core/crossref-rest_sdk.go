...
func (sdk *CrossrefRestSDK) OptionsMap() map[string]any {
...
func (sdk *CrossrefRestSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *CrossrefRestSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *CrossrefRestSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *CrossrefRestSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *CrossrefRestSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *CrossrefRestSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("CrossrefRestSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *CrossrefRestSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *CrossrefRestSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("CrossrefRestSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *CrossrefRestSDK {
	...
	sdk := NewCrossrefRestSDK(sdkopts)
	sdk.Mode = "test"
