// Command CrossrefRest-cli is a CLI + REPL for the CrossrefRest SDK,
...
const prompt = "crossref-rest"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "funder journal member type work"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CROSSREF_REST_APIKEY (api key), CROSSREF_REST_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CROSSREF_REST_APIKEY carries the API key and
	// CROSSREF_REST_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CROSSREF_REST_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CROSSREF_REST_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCrossrefRestSDK(opts)
