...
boru-driven command-line client **and** interactive REPL for the CrossrefRest
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/crossref-rest-cli)
make build

# 2. See usage (words, entities, env vars)
./crossref-rest-cli --help

# 3. Provide credentials once, via the environment
export CROSSREF_REST_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./crossref-rest-cli load 1 funder            # {id:1} shorthand
./crossref-rest-cli load '{id:1}' funder       # explicit match map

# 5. Override the API base URL for a single call
CROSSREF_REST_BASE=https://api.example.com ./crossref-rest-cli load 1 funder

# 6. No arguments -> interactive REPL
./crossref-rest-cli
crossref-rest> load 1 funder
crossref-rest> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/crossref-rest-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/crossref-rest-cli load 1 funder
   ```
...
```sh
./crossref-rest-cli load 1 funder          # scalar shorthand for {id:1}
./crossref-rest-cli load '{id:1}' funder     # explicit match map
```
...
./crossref-rest-cli load 1 funder
```
...
Run with no arguments to open a REPL (prompt `crossref-rest>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `funder`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/crossref-rest-cli`. |
...
(`./crossref-rest-cli <expr>`) and interactively (the REPL), and expressions compose the
