...
module CrossrefRestOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if CrossrefRestOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = CrossrefRestOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| CrossrefRestOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || CrossrefRestOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = CrossrefRestSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = CrossrefRestError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { CrossrefRestOmni.errify(err) }
