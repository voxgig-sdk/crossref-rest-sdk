...
module CrossrefRestUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CrossrefRestResponse.new({})
    ...
    if fetch_err
      ...
      response = CrossrefRestResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CrossrefRestResponse.new(fetched)
    else
