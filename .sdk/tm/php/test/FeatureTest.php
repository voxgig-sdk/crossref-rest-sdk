...
class FtCtrl extends CrossrefRestControl
{
...
class FtClient
{
    ...
    private CrossrefRestUtility $utility;

    public function __construct(CrossrefRestUtility $utility, array $options)
    {
    ...
    public function get_utility(): CrossrefRestUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CrossrefRestContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CrossrefRestContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CrossrefRestContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CrossrefRestConfig::make_feature($name);
    ...
    private static function buildUrl(CrossrefRestSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CrossrefRestError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CrossrefRestResponse($fetched);
        }
    ...
    private function fail(CrossrefRestContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CrossrefRestContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
