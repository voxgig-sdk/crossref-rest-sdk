...
class TestFunderDirect:

    def test_should_direct_load_funder(self):
        setup = _funder_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-funder", "live" if setup["live"] else "unit")
        if _skip:
...
def _funder_direct_setup(mockres):
