    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CROSSREFREST_TEST_JOURNAL_ENTID JSON to run live")
        client = setup["client"]
        ...
        journal_ref01_data_dt0_loaded, err = journal_ref01_ent.load(journal_ref01_match_dt0, None)
        assert err is None
...
def _journal_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CROSSREFREST_TEST_JOURNAL_ENTID"))
