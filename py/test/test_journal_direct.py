...
class TestJournalDirect:
    ...
    def test_should_direct_load_journal(self):
        setup = _journal_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-journal", "live" if setup["live"] else "unit")
        if _skip:
