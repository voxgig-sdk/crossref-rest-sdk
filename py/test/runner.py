...
class CrossrefRestTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CrossrefRestTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CrossrefRestTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CrossrefRestTestRunner.getenv("CROSSREF_REST_TEST_LIVE")
        override = CrossrefRestTestRunner.getenv("CROSSREF_REST_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CrossrefRestTestRunner.getenv(key)
        ...
        explain = CrossrefRestTestRunner.getenv("CROSSREF_REST_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if CrossrefRestTestRunner._test_control is not None:
            return CrossrefRestTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CrossrefRestTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CrossrefRestTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CrossrefRestTestRunner.load_test_control()
...
def load_env_local():
    CrossrefRestTestRunner.load_env_local()


def env_override(m):
    return CrossrefRestTestRunner.env_override(m)


def entity_data(v):
    return CrossrefRestTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CrossrefRestTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CrossrefRestTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CrossrefRestTestRunner.load_test_control()


def live_client_options():
    return CrossrefRestTestRunner.live_client_options()


def live_delay_ms():
    return CrossrefRestTestRunner.live_delay_ms()
